...
// Forward declaration for request handler
...
typedef struct {
    ...
    RequestHandler *handler; // Handler for this stream
} stream_data;
...
void api_request_handler(RequestHandler *self, 
    ...
    // Set response body
    if (sdata->body) free(sdata->body);
    sdata->body = strdup(json);
    sdata->body_len = json_len;
    sdata->body_offset = 0;
...
void root_request_handler(RequestHandler *self, 
    ...
    // Set response body
    if (sdata->body) free(sdata->body);
    sdata->body = strdup(html);  // stddup复制的字符串需要通过free释放
    sdata->body_len = html_len;
    sdata->body_offset = 0;
...
static RequestHandler default_handler_impl = {
    .handle_request = default_request_handler,
    .data = NULL
};

static RequestHandler api_handler_impl = {
    .handle_request = api_request_handler,
    .data = NULL
};

static RequestHandler root_handler_impl = {
